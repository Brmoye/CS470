...
class TwoPlayersGame:
    ...
    def play(self, nmoves=1000, verbose=True):
        ...
        for self.nmove in range(1, nmoves+1):
            ...
            if self.is_over():  # type: ignore
                break
            
            move = self.player.ask_move(self)
            history.append((deepcopy(self), move))
            self.make_move(move)  # type: ignore
    ...
    @property
    def player(self):
        return self.players[self.nplayer- 1]  # type: ignore
    
    @property
    def opponent(self):
        return self.players[self.nopponent - 1]  # type: ignore
